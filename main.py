...
def predict():
    if request.method == 'POST':
        ...
        if not predicted_disease:
            return render_template('index.html', predicted_disease="No disease found")
        ...
        for i in pre[0]:
            my_pre.append(i)
        dis_med = [med.replace("'", "").replace("[", "").replace("]", "").strip() for med in med]

        my_die = []
        for i in die:
            my_die.append(i)
        my_diet = [my_die.replace("'", "").replace("'", "").replace("[", "").replace("]", "").strip() for my_die in my_die]



    return render_template('index.html',predicted_disease=predicted_disease,desc=desc,dis_pre=my_pre,dis_med=dis_med,dis_diet=my_diet,dis_wrkout=wrkout)
@app.route('/about')
def about():
